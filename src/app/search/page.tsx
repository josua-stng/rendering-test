...
export default function SearchDataProduct({ searchParams }: SearchPageProps) {
  ...
  const filterData = products?.filter((dataProducts: { title: string }) => {
    ...
    <div>
      {queryNotFound ? (
        ...
        <div className="max-w-6xl grid grid-cols-3 items-start justify-center mx-auto rounded-md gap-5">
          {filterData.map((product: propsProduct) => {
            return (
              <Link
                key={product.id}
              ...
              >
                ...
                <p className="line-clamp-1 mb-2 font-semibold">
                  {product.title}
                </p>
